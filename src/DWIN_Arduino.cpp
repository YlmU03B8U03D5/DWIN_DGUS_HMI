...
{ //  HEX(5A A5 04 83 00 0F 01)
    ...
    i2cuart.write(sendBuffer, sizeof(sendBuffer));
    delay(10);
...
// Function to read incoming data from DWIN display and print to Serial
void DWIN::readAndPrintIncomingData() {
    String response = readDWIN(); // Read data from DWIN
    if (response.length() > 0) {
        Serial.println("Incoming Data: " + response); // Print the incoming data
    }
}

// Modify the listen function to include reading and printing
void DWIN::listen() {
    handle();    
    readAndPrintIncomingData(); // Call the new function to read and print data
}
